...
#define FAULT (1<<4)
#define PWM (1<<4)
#define MIN_PWM (51) // 20%

static unsigned int grp2_addr = 0;
module_param(grp2_addr, uint, S_IRUGO);
MODULE_PARM_DESC(grp2_addr, "The group with only 2 fans (0 by default: 4 fans)");
...
static const unsigned short normal_i2c[] = {0x0a, 0x1a, 0x29, 0x2a, I2C_CLIENT_END};
...
static ssize_t get_fan(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	u32 tach1;
	u32 tach2;
	int index = (attr->index & ~FAULT);

	for (i = 0; i < 10; i++) {
		tach1 = i2c_smbus_read_byte_data(client, tach_reg[index]);
		if (tach1 >= 0)
			break;
	}
	tach1 = (tach1 << 3) & 0x7f8;
	for (i = 0; i < 10; i++) {
		tach2 = i2c_smbus_read_byte_data(client, tach_reg[index] + 1);
		if (tach2 >= 0)
			break;
	}
	tach2 = (tach2 >> 5) & 0x7;
	...
	if (attr->index & FAULT) {
		/* special for max6629: fake fan_fault */
...
static ssize_t get_target(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	u32 target1;
	u32 target2;
	int index = (attr->index & ~PWM);

	target1 = i2c_smbus_read_byte_data(client, target_reg[index]);
	target1 = (target1 << 3) & 0x7f8;
	target2 = i2c_smbus_read_byte_data(client, target_reg[index] + 1);
...
static ssize_t set_target(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	target2 = (target & 0x07) << 5;
	i2c_smbus_write_byte_data(client, target_reg[index], target1);
	i2c_smbus_write_byte_data(client, target_reg[index] + 1, target2);
...
static SENSOR_DEVICE_ATTR(fan1_fault, S_IRUGO, get_fan, NULL, 0|FAULT);
static SENSOR_DEVICE_ATTR(fan2_fault, S_IRUGO, get_fan, NULL, 1|FAULT);
static SENSOR_DEVICE_ATTR(fan3_fault, S_IRUGO, get_fan, NULL, 2|FAULT);
static SENSOR_DEVICE_ATTR(fan4_fault, S_IRUGO, get_fan, NULL, 3|FAULT);
static SENSOR_DEVICE_ATTR(fan1_input, S_IRUGO, get_fan, NULL, 0);
static SENSOR_DEVICE_ATTR(fan2_input, S_IRUGO, get_fan, NULL, 1);
static SENSOR_DEVICE_ATTR(fan3_input, S_IRUGO, get_fan, NULL, 2);
static SENSOR_DEVICE_ATTR(fan4_input, S_IRUGO, get_fan, NULL, 3);
static SENSOR_DEVICE_ATTR(pwm1, S_IWUSR | S_IRUGO, get_target, set_target, 0|PWM);
static SENSOR_DEVICE_ATTR(pwm2, S_IWUSR | S_IRUGO, get_target, set_target, 1|PWM);
static SENSOR_DEVICE_ATTR(pwm3, S_IWUSR | S_IRUGO, get_target, set_target, 2|PWM);
static SENSOR_DEVICE_ATTR(pwm4, S_IWUSR | S_IRUGO, get_target, set_target, 3|PWM);
...
static struct attribute *max6620_attrs2[] = {
	&sensor_dev_attr_fan1_fault.dev_attr.attr,
	&sensor_dev_attr_fan2_fault.dev_attr.attr,
	&sensor_dev_attr_fan1_input.dev_attr.attr,
	&sensor_dev_attr_fan2_input.dev_attr.attr,
...
static struct attribute_group max6620_attr_grp2 = {
	.attrs = max6620_attrs2,
};
...
static int max6620_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->addr == grp2_addr) {
		grp = &max6620_attr_grp2;
	} else {
		grp = &max6620_attr_grp;
	}
	...
	err = sysfs_create_group(&client->dev.kobj, grp);
	...
	sysfs_remove_group(&client->dev.kobj, grp);
	return err;
}
...
static int max6620_init_client(struct i2c_client *client)
{
	...
	for (i = 0; i < 4; i++) {
		...
		/* default value=0x4c sr=0x4, 0xc means rpm mode */
