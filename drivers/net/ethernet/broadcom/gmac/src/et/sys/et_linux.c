...
static void et_watchdog(struct timer_list *t);
#ifdef GMAC_ALL_PASSIVE
...
static void
et_watchdog(struct timer_list *t)
{
	et_info_t *et = from_timer(et, t, timer);
	struct net_device *dev = et->dev;
...
/* Rate limiting */
#ifdef GMAC_RATE_LIMITING
static void et_release_congestion(struct timer_list *t) 
{
	etc_info_t *etc = from_timer(etc, t, rl_timer);
...
#endif
	if (etc->rl_stopping_broadcasts) {
		etc->rl_stopping_broadcasts = 0;
		/* Clear the number of dropped broadcast packets */
		etc->rl_dropped_bc_packets = 0;
	}
	if (etc->rl_stopping_all_packets) {
		etc->rl_stopping_all_packets = 0;
		etc->rl_dropped_all_packets = 0;
	}
}
#endif /* GMAC_RATE_LIMITING */
...
static int iproc_gmac_drv_probe(struct platform_device* pldev)
{
	...
	timer_setup(&et->timer, et_watchdog, 0);
...
#ifdef GMAC_RATE_LIMITING
	/* Init 1 second watchdog timer */
	timer_setup(&et->etc->rl_timer, et_release_congestion, 0);
#endif /* GMAC_RATE_LIMITING */
