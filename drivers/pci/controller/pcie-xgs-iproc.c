...
static int iproc_pcie_probe(struct platform_device *pdev)
{
	...
	ret = devm_of_pci_get_host_bridge_resources(pcie->dev, 0, 0xff, &res, &iobase);
	if (ret) {
		dev_err(pcie->dev,
...
//	if (of_device_is_compatible(np, "iproc-p2") || of_device_is_compatible(np, "iproc-p6")) {
  		WrongPCIGen2TemplateWAR(of_get_pci_domain_nr(np), 0x13, 0x190);
		WrongPCIGen2TemplateWAR(of_get_pci_domain_nr(np), 0x19, 0x191);
// 	}
#endif
...
static int iproc_pcie_platform_remove(struct platform_device *pdev)
{
...
static struct platform_driver iproc_pcie_pltfm_driver = {
	...
	.remove = iproc_pcie_platform_remove,
};
