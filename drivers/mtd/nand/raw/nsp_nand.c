...
#define NAND_STRAP_WIDTH						     \
	((readl(ctrl.nand_strap_regs) & ctrl.data->strap_width_bitfield.mask)\
	>> ctrl.data->strap_width_bitfield.shift)
...
#if defined(CONFIG_ARCH_NORTHSTAR2)
/* Northstar-2 specific data */
static const struct nand_ctrl_data ns2_nand_ctrl_data = {
	.chip_select_max = 1,
	.strap_type_bitfield = {
		.mask = 0x000f0000,
		.shift = 16,
 ...
 */
static int iproc_ooblayout_ecc(struct mtd_info *mtd, int section,
			      struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct iproc_nand_host *host = nand_get_controller_data(chip);
	struct iproc_nand_cfg *cfg = &host->hwcfg;
	int sas = cfg->spare_area_size << cfg->sector_size_1k;
	int sectors = cfg->page_size / (512 << cfg->sector_size_1k);

	if (sectors > MTD_MAX_OOBFREE_ENTRIES)
		sectors = MTD_MAX_OOBFREE_ENTRIES;
...
static int iproc_ooblayout_free(struct mtd_info *mtd, int section,
			       struct mtd_oob_region *oobregion)
{
	struct nand_chip *chip = mtd_to_nand(mtd);
	struct iproc_nand_host *host = nand_get_controller_data(chip);
...
static const struct mtd_ooblayout_ops iproc_ooblayout_ops = {
	.ecc = iproc_ooblayout_ecc,
	.free = iproc_ooblayout_free,
};
...
{
	pr_debug("%s: native cmd %d addr_lo 0x%08x\n", __func__, cmd,
		 NAND_REG_RD(NCREG_CMD_ADDRESS));
...
static int iproc_nand_waitfunc(struct mtd_info *mtd, struct nand_chip *this)
{
	...
	if (ctrl.cmd_pending &&
		...
		dev_err(&host->pdev->dev,
			"timeout waiting for command %u (%d)\n",
			host->last_cmd,
			NAND_REG_RD(NCREG_CMD_START) >> 24);
		dev_err(&host->pdev->dev,
			"irq status %08x, intfc status %08x\n",
			NAND_TEST_IRQ(NCINTR_CTLRDY),
		...
		u32 *buf, u8 *oob)
{
	...
	if (max_bitflips) {
		pr_debug("%s: corrected %d bit errors at 0x%llx\n",
			 __func__, max_bitflips, start_addr);
...
static int
iproc_nand_read_subpage(struct mtd_info *mtd, struct nand_chip *chip,
...
int iproc_nand_write_page_raw(struct mtd_info *mtd, struct nand_chip *chip,
...
static void iproc_nand_timing_setup(struct iproc_nand_host *host)
{
	...
	if (chip->parameters.onfi->version) {
		onfi_tmode = le16_to_cpu(get_unaligned(
				(u16 *)&chip->parameters.onfi->async_timing_mode));
...
static int nspnand_attach_chip(struct nand_chip *chip)
{
...
static const struct nand_controller_ops nspnand_controller_ops = {
	.attach_chip = nspnand_attach_chip,
};
...
static int iproc_nand_probe(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
	pr_info("%s: iproc_nand_ctrl_setup ret %d\n", __func__, ret);
	...
	mtd_set_ooblayout(mtd, &iproc_ooblayout_ops);

	chip->controller = &ctrl.controller;

	chip->options |= NAND_BUSWIDTH_AUTO;

	chip->controller->ops = &nspnand_controller_ops;
	ret = nand_scan(mtd, ctrl.max_cs);
	if (ret)
		goto err1;
...
static int iproc_nand_ctrl_setup(struct platform_device *pdev)
{
	...
	ctrl.strap_bus_width = NAND_STRAP_WIDTH;
	...
	/* Get bus width property */
	ctrl.bus_width = 8;
	of_property_read_u32(dn, "bus-width", &ctrl.bus_width);
	if (ctrl.bus_width != 8 && ctrl.bus_width != 16) {
		dev_warn(&pdev->dev,
			 "invalid bus-width property %d (using 8)\n",
			 ctrl.bus_width);
		ctrl.bus_width = 8;
	}
	...
	pr_debug("%s: bus_width %d\n", __func__, ctrl.bus_width);
	...
	/* Force both CS to 8-bit width at init for reading correct dev ID */
	NAND_REG_CLR(NCREG_CONFIG_CS0, NCFLD_CONFIG_CS0_DEVICE_WIDTH_MASK);
	NAND_REG_CLR(NCREG_CONFIG_CS1, NCFLD_CONFIG_CS0_DEVICE_WIDTH_MASK);
